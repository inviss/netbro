...
/**
 * 카테고리관련 조회함수의 where 조건을 생성하는 함수.
 * @author asura
 *
 */
public class CategorySpecifications {
	...
	public static Specification<CategoryTbl> findListForOderNumByBetWeen( final Integer startOrder , final Integer endOrder) {

		Specifications<CategoryTbl> specifications = where(OrderNumBetween(startOrder,endOrder));

		return specifications;

	}

	public static Specification<CategoryTbl> findListForOderNumByGreaterThan( final Integer startOrder  ) {

		Specifications<CategoryTbl> specifications = where(OrderNumGreaterThan(startOrder));
	...
	public static Specification<CategoryTbl> DepthEqual(final Integer depth) {
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				if(depth != null){
					return cb.equal(newCategoryRoot.get(CategoryTbl_.depth), depth);
				}else return null;
			}
		};
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				if(categoryId != null && categoryId > 0){
					return cb.equal(newCategoryRoot.get(CategoryTbl_.preParent), categoryId);
				} else {
					return cb.equal(newCategoryRoot.get(CategoryTbl_.depth), 0);
				}
			}
		};
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(StringUtils.isNotBlank(nodes)){
					...
					return cb.like(newCategoryRoot.get(CategoryTbl_.nodes), likePattern);
				}
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(orderNum != null){
					...
					return cb.greaterThanOrEqualTo(newCategoryRoot.get(CategoryTbl_.orderNum), orderNum);
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(orderNum != null){
					...
					return cb.lessThanOrEqualTo(newCategoryRoot.<Integer>get(CategoryTbl_.orderNum), orderNum);
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(startOrder != null && endOrder != null){
					...
					return cb.and(cb.greaterThanOrEqualTo(newCategoryRoot.get(CategoryTbl_.orderNum), startOrder), cb.lessThan(newCategoryRoot.get(CategoryTbl_.orderNum), endOrder));
	 ...
	 * @return CriteriaBuilder
	 */
	public static Specification<CategoryTbl> OrderNumGreaterThan(final Integer startOrder) {
		...
		return new Specification<CategoryTbl>() {
			...
			public Predicate toPredicate(Root<CategoryTbl> newCategoryRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(startOrder != null ){
					...
					return cb.greaterThanOrEqualTo(newCategoryRoot.get(CategoryTbl_.orderNum), startOrder);
