...
public class CodeSpecifications {
	 ...
	 * @return CriteriaBuilder
	 */
	public static Specification<CodeTbl> Rmk1Equal(final String clfCd, final String rmk1) {
		...
		return new Specification<CodeTbl>() {
			...
			public Predicate toPredicate(Root<CodeTbl> CodeRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(StringUtils.isNotBlank(rmk1)){
					return cb.and(cb.equal(CodeRoot.<String>get("id").get("clfCD"), clfCd), cb.equal(CodeRoot.<String>get("rmk1"), rmk1));
				}
