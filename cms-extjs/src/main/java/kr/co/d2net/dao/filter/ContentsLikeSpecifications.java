...
public class ContentsLikeSpecifications {
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(startDt != null && endDt != null) {
					...
					return cb.and(cb.greaterThanOrEqualTo(root.get(ContentsTbl_.regDt), startDt), 
							cb.and(cb.lessThanOrEqualTo(root.get(ContentsTbl_.regDt), endDt)));
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(CategryId != 0) {
					...
					return cb.equal(root.get(ContentsTbl_.categoryId), CategryId);
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(id != null) {
					...
					return cb.and(cb.equal(root.get(ContentsTbl_.categoryId), id.getCategoryId()),cb.equal(root.get(ContentsTbl_.episodeId), id.getEpisodeId()));
	 ...
	 * @return
	 */
	public static Specification<ContentsTbl> equalEpisode(final long categoryId) {
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(categoryId != 0) {
					...
					return cb.and(cb.equal(root.get(ContentsTbl_.categoryId), categoryId));
	...
	public static Specification<ContentsInstTbl> ContentsNmLike(Search search) {
		...
		return new Specification<ContentsInstTbl>() {
			...
			public Predicate toPredicate(Root<ContentsInstTbl> ContentsRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				return cb.like(cb.lower(ContentsRoot.get(ContentsInstTbl_.ctiFmt)), likePattern);
	...
	public static Specification<ContentsTbl> deleteRangeSearch(final Search search) {
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> ContentsRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				return cb.and(cb.equal(ContentsRoot.get(ContentsTbl_.useYn), "Y"), 
						cb.lessThanOrEqualTo(ContentsRoot.get(ContentsTbl_.delDd), search.getToDate()));
	...
	public static Specification<ContentsInstTbl> findContentIntByCtId(final Long ctId) {
		...
		return new Specification<ContentsInstTbl>() {
			...
			public Predicate toPredicate(Root<ContentsInstTbl> ContentsRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				return cb.equal(ContentsRoot.get(ContentsInstTbl_.ctId), ctId);
			}
		...
		return new Specification<ContentsTbl>() {
			...
			public Predicate toPredicate(Root<ContentsTbl> ContentsRoot, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(StringUtils.isNotBlank(search.getDataStatCd())){
					return cb.equal(ContentsRoot.get(ContentsTbl_.dataStatCd), search.getDataStatCd());
