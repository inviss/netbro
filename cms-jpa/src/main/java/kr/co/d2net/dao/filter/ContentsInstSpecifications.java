...
public class ContentsInstSpecifications {

	private final static Logger logger = LoggerFactory.getLogger(ContentsInstSpecifications.class);
	...
	public static Specification<ContentsInstTbl> findContentInst(final ContentsInst contentsInst) {
		return new AbstractSpecification<ContentsInstTbl> () {
			@Override
			public Predicate toPredicate(CriteriaBuilder cb, CriteriaQuery<?> cq, Root<ContentsInstTbl> root) {
			   
				Predicate p1 = cb.conjunction();
				if(contentsInst.getCtId() != null && contentsInst.getCtId() > 0) {
					if(logger.isDebugEnabled()) {
						logger.debug("contentsInst[ctId]: "+contentsInst.getCtId());
					}
					p1 = cb.equal(root.get(ContentsInstTbl_.ctId), contentsInst.getCtId());
				}
				
				Predicate p2 = cb.conjunction();
				if(StringUtils.isNotEmpty(contentsInst.getCtiFmt())) {
					if(logger.isDebugEnabled()) {
						logger.debug("contentsInst[ctiFmt]: "+contentsInst.getCtiFmt());
					}
					p2 = cb.like(root.get(ContentsInstTbl_.ctiFmt), contentsInst.getCtiFmt()+"%");
				}
				
				Predicate p3 = cb.conjunction();
				if(contentsInst.getCtiId() != null && contentsInst.getCtiId() > 0) {
					if(logger.isDebugEnabled()) {
						logger.debug("contentsInst[ctiId]: "+contentsInst.getCtiId());
					}
					p3 = cb.equal(root.get(ContentsInstTbl_.ctiId), contentsInst.getCtiId());
				}
