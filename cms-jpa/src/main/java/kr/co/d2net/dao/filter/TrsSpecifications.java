...
public class TrsSpecifications {
	...
	public static Specification<TrsTbl> findContentsByExists() {
		return new AbstractSpecification<TrsTbl> () {
			@Override
			public Predicate toPredicate(CriteriaBuilder cb, CriteriaQuery<?> cq, Root<TrsTbl> root) {
	...
	@SuppressWarnings("serial")
	public static Specification<TrsTbl> findContentsOnlyByParams(final Search search) {
		return new AbstractSpecification<TrsTbl> () {
			@Override
			public Predicate toPredicate(CriteriaBuilder cb, CriteriaQuery<?> cq, Root<TrsTbl> root) {
				...
				if(search.getCategoryId() != null && search.getCategoryId() > 0) {
					p1 = cb.equal(root.get(TrsTbl_.busiPartnerId), search.getCategoryId());
				}

				// episode_id
				Predicate p2 = cb.conjunction();
				if(search.getEpisodeId() != null && search.getEpisodeId() > 0) {
					p2 = cb.equal(root.get(TrsTbl_.busiPartnerId), search.getEpisodeId());
				}

				// segment_id
				Predicate p3 = cb.conjunction();
				if(search.getSegmentId() != null && search.getSegmentId() > 0) {
					p3 = cb.equal(root.get(TrsTbl_.busiPartnerId), search.getEpisodeId());
				}
				...
				if(search.getOperator() == Operator.LIST) {
					Path<Long> ctId = root.get(TrsTbl_.busiPartnerId);
