...
public class Hnc2String {
	...
	static {
		...
		Pattern UINT16MAP = Pattern.compile("0x([0-9a-f]{4})", Pattern.CASE_INSENSITIVE);
		Pattern UINT32MAP = Pattern.compile("\\[0x([0-9a-f]{2,4})\\] = 0x([0-9a-f]{2,6})", Pattern.CASE_INSENSITIVE);
		...
		InputStream resource = Hnc2String.class.getResourceAsStream("hnc2unicode.inc");
		try {
			...
			for (;;) {
				...
				if (status == 1) {
					if ("};".equals(line)) {
						status = 0;
						log.debug("ksc5601_2uni_page4a 매핑 읽기 종료({}) {}건", lineNumber, count);
					} else {
	...
	public static String convert(int c) {
		...
		if (c >= 0x0020 && c <= 0x007e) {
			...
			log.trace("ASCII printable (c >= 0x0020 && c <= 0x007e) {} => {}", c, s);
